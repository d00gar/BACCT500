...
/*
 ...
 foreach x in $B1 $all_r1 {
 levelsof `ind', l(ind_level) 
 foreach level of local ind_level {
 graph box `x' if `ind'==`level', over(fyear)  ///
 ...
 * now a more complicated story ... "designing a picture to tell a story"
 ...
 levelsof `ind', l(ind_level) 
 foreach level of local ind_level {
 di "ind =" `"`ind'"' ", plotted var =" `"`x'"' ", level =" `level'
 summ upper lower uhinge lhinge median mean if `ind'==`level'
 twoway (rbar lhinge uhinge fyear if `ind'==`level' & fyear!=. & tic!="", fcolor(gs12) barwidth(0.6)) ///
	   ...
	   (scatter at fyear if `ind'==`level' & tic!="" & fyear!=. & `x'>uhinge & `x'<upper, sort mcolor(dkgreen) msize(tiny) msymbol(square)) ///
	   (scatter at fyear if `ind'==`level' & tic!="" & fyear!=. & `x'>upper, sort mcolor(blue) msize(tiny) msymbol(lgx)), ///
	   ytitle("`:variable label `x''") ylabel(, labsize(small) angle(horizontal)) ymtick(, angle(horizontal)) ///
...
twoway (rbar lhinge uhinge fyear if sic2==73 & fyear!=. & tic!="", fcolor(gs12) barwidth(0.6)) ///
	   ...
	   (scatter at fyear if sic2==73 & tic!="" & fyear!=. & `x'>uhinge & `x'<upper, sort mcolor(dkgreen) msize(tiny) msymbol(square)) ///
	   (scatter at fyear if sic2==73 & tic!="" & fyear!=. & `x'>upper, sort mcolor(blue) msize(tiny) msymbol(lgx)), ///
